...
void MainWindow::createNewSequence()
{
    ...
    case 0: // MutableArray
        ...
        else if (currentDataType == 1) currentDoubleSequence = new MutableArraySequence<double>();
        else currentStringSequence = new MutableArraySequence<QString>();
        break;
    case 1: // MutableList
        if (currentDataType == 0) currentIntSequence = new MutableListSequence<int>();
        else if (currentDataType == 1) currentDoubleSequence = new MutableListSequence<double>();
        else currentStringSequence = new MutableListSequence<QString>();
        break;
    case 2: // ImmutableArray
        if (currentDataType == 0) currentIntSequence = new ImmutableArraySequence<int>();
        else if (currentDataType == 1) currentDoubleSequence = new ImmutableArraySequence<double>();
        else currentStringSequence = new ImmutableArraySequence<QString>();
        break;
    case 3: // ImmutableList
        if (currentDataType == 0) currentIntSequence = new ImmutableListSequence<int>();
        else if (currentDataType == 1) currentDoubleSequence = new ImmutableListSequence<double>();
        else currentStringSequence = new ImmutableListSequence<QString>();
        break;
    }
}
...
void MainWindow::on_appendButton_clicked()
{
    ...
    try {
        if (isBitMode && currentBitSequence) {
            ...
            Sequence<Bit>* newSeq = currentBitSequence->Append(b);
            delete currentBitSequence;
            ...
            if (currentDataType == 0 && currentIntSequence) {
                ...
                else if (mList) mList->Append(val);
                else {
                ...
                if (!ok) throw std::invalid_argument("Не число с плавающей точкой");
    ...
    try {
        if (isBitMode && currentBitSequence) {
            ...
            if (currentDataType == 0 && currentIntSequence) {
                ...
                MutableArraySequence<int>* mArr = dynamic_cast<MutableArraySequence<int>*>(currentIntSequence);
                MutableListSequence<int>* mList = dynamic_cast<MutableListSequence<int>*>(currentIntSequence);
                if (mArr) mArr->Prepend(val);
                else if (mList) mList->Prepend(val);
                else {
    ...
    try {
        if (isBitMode && currentBitSequence) {
            ...
            if (currentDataType == 0 && currentIntSequence) {
                ...
                MutableArraySequence<int>* mArr = dynamic_cast<MutableArraySequence<int>*>(currentIntSequence);
                MutableListSequence<int>* mList = dynamic_cast<MutableListSequence<int>*>(currentIntSequence);
                if (mArr) mArr->InsertAt(val, index);
                else if (mList) mList->InsertAt(val, index);
                else {
